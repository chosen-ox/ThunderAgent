# ThunderAgent + Qwen3-8B + mini-SWE-agent on A100

## Architecture

```
mini-SWE-agent --> ThunderAgent (port 9000) --> vLLM (port 8000, A100 GPU)
```

- **vLLM**: Model inference backend, loads Qwen3-8B on A100 GPU
...
```bash
OUTPUT_DIR="results/qwen3_8b_lite"
mkdir -p $OUTPUT_DIR
```
...
Single A100 80GB is sufficient for Qwen3-8B (~16GB BF16):
...
Note: vLLM logs to stdout/stderr only (no log file by default). Use `| tee` to capture to file while still seeing output.
...
With `--profile-dir $OUTPUT_DIR`, the `step_profiles.csv` file is written directly into the output directory during the run.
...
- Increase if GPU utilization is low; decrease if OOM occurs
...
Example from Qwen3-8B A100 run:

```
Block-level hit rate: 25.2%
Token-level reuse:    70.8%  (24.5M / 34.7M prompt tokens reused)
```

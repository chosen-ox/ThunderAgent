#!/usr/bin/env bash
#
# Sequential: Baseline (vLLM only) then ThunderAgent, 48 workers, 100 tasks.
#
# Usage:
#   bash scripts/run_0425_48_sequential.sh
...
DIR_BASELINE="results/qwen3_8b_lite_48_0425_vllm"
DIR_THUNDER="results/qwen3_8b_lite_48_0425_thunder"
...
run_baseline() {
  echo ""
  echo "════════════════════════════════════════════════════════════════════"
  echo "  BASELINE: 100 tasks, ${WORKERS} workers → ${DIR_BASELINE}"
  echo "════════════════════════════════════════════════════════════════════"
  echo ""
  ...
  echo ""
  echo "[RUN] Starting mini-extra swebench (100 tasks, ${WORKERS} workers) ..."
  echo ""
  ...
  curl -sf "http://localhost:${BASELINE_PORT}/metrics" 2>/dev/null \
    > "${DIR_BASELINE}/vllm_metrics.txt" 2>/dev/null || echo "[WARN] Failed to dump vLLM metrics"
...
run_thunderagent() {
  echo ""
  echo "════════════════════════════════════════════════════════════════════"
  echo "  THUNDERAGENT: 100 tasks, ${WORKERS} workers → ${DIR_THUNDER}"
  echo "════════════════════════════════════════════════════════════════════"
  echo ""
  ...
  curl -sf "http://localhost:${TA_PORT}/metrics" 2>/dev/null | python3 -m json.tool \
    > "${DIR_THUNDER}/thunderagent_metrics.json" 2>/dev/null || echo "[WARN] Failed to dump TA metrics"
  curl -sf "http://localhost:${TA_PORT}/health" 2>/dev/null | python3 -m json.tool \
    > "${DIR_THUNDER}/thunderagent_health.json" 2>/dev/null || echo "[WARN] Failed to dump TA health"
  curl -sf "http://localhost:${VLLM_PORT}/metrics" 2>/dev/null \
    > "${DIR_THUNDER}/vllm_metrics.txt" 2>/dev/null || echo "[WARN] Failed to dump vLLM metrics"
...
echo " 48 workers, 100 tasks each"
echo " $(date)"
echo "=============================================="
